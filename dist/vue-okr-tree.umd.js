...
/***/ (function (module, exports, __webpack_require__) {
        ...
        // to indexed object, toObject with fallback 红达 for non-array-like ES3 strings
...
/* harmony default export */ var OkrTreeNodevue_type_script_lang_js_ = ({
          ...
          methods: {
            ...
            leftChildNodesArrClose: function leftChildNodesArrClose(array) {
              if (array.length == 0) {
                return
              }
              for (let i = 0; i < array.length; i++) {
                if (array[i].leftExpanded) {
                  if (this.node != array[i]) {
                    array[i].leftExpanded = false
                  }
                }
                this.leftChildNodesArrClose(array[i].leftChildNodes)
              }
            },
            childNodesArrClose : function childNodesArrClose(array) {
              if (array.length == 0) {
                return
              }
              for (let i = 0; i < array.length; i++) {
                ...
                this.childNodesArrClose(array[i].childNodes)
              }
            },
            handleBtnClick: function handleBtnClick(isLeft) {
              ...
              if (store.onlyBothTree) {
                ...
                this.leftChildNodesArrClose(this.childNodesArr)
                this.childNodesArrClose(this.childNodesArr);
                this.childNodesArr = childNodes;
                // console.log(this.childNodesArr,'2222')
                if (isLeft) {
                  let leftData = this.node.leftData ? this.node.leftData : this.node.data;
                  if (this.node.leftExpanded) {
                    this.node.leftExpanded = false;
                    // console.log('关闭',this.node)
                    this.leftChildNodesArrClose(this.node.leftChildNodes)
                    this.tree.$emit("node-collapse", leftData, this.node, this, isLeft);
                  } else {
                    this.node.leftExpanded = true;
                    this.tree.$emit("node-expand", leftData, this.node, this, isLeft);
                  }
                  return;
                }
              }
              ...
              if (this.node.expanded) {
                ...
                this.node.collapse()
                this.childNodesArrClose(this.node.childNodes);
                this.tree.$emit("node-collapse", this.node.data, this.node, this, isLeft);
              } else {
                this.node.expand();
                this.tree.$emit("node-expand", this.node.data, this.node, this, isLeft);
              }
            },
              ...
              if (this.leftData) {
                ...
                if (this.isLeftChilds) {
                  for (let i = 0; i < this.root.childNodes.length; i++) {
                    this.root.childNodes[i].leftData = this.isLeftChilds.childNodes[i].data;
